(function () {
    ...
    function hashToLink(doclet, hash) {
        ...
        if (!linkRegExp.test(hash)) {
            return hash;
        }
        url = helper.createLink(doclet);
        url = url.replace(linkRegExpReplace, hash);
        return '<a href="' + url + '">' + hash + '</a>';
    }

    function needsSignature(doclet) {
        var needsSig = false, i, l;
    ...
    function buildNavForSearch(members) {
        var nav = [];

        Object.keys(members).forEach(function (type) {
            if (!members[type] || !members[type].length) {
                return;
            }

            members[type].forEach(function (v) {
                nav.push({
                    type: type,
                    longname: v.longname,
                    name: v.name,
    ...
    exports.publish = function (taffyData, opts, tutorials) {
        ...
        outputSourceFiles = conf.default && conf.default.outputSourceFiles !== false ? true : false;
        ...
        view.allMembers = buildNavForSearch(members);
        view.allMemberTypes = Object.keys(members);
